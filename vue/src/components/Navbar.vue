        ...
        <ul class="flex flex-col space-y-6 mt-16">
          <li v-for="link in navLinks" :key="link.label">
            <a
              ...
              class="text-gray-800 text-lg font-medium hover:text-gray-700 hover:border-b-2 hover:border-gray-700 pb-1 transition-all duration-200"
            >
              {{ link.label }}
            </a>
            <a
    ...
    <ul class="hidden sm:flex md:flex space-x-6 items-center mr-10 mt-4 sm:mt-0">
      <li v-for="link in navLinks" :key="link.label" class="px-4">
        <a
          ...
          :href="link.href"
          :class="{'border-b-2 border-gray-700': isActive(link.label)}"
          class="text-gray-800 font-bold hover:text-gray-700 hover:border-b-2 hover:border-gray-700 pb-1 transition-all duration-200"
        >
          {{ link.label }}
        </a>
        <a
